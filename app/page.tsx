...
function Reveal({ children, delay = 0, className = '' }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <div
      ...
      style={
        inView
          ? { opacity: 1, transform: 'translateY(0)', transition: `opacity 0.7s cubic-bezier(0.22,1,0.36,1) ${delay}ms, transform 0.7s cubic-bezier(0.22,1,0.36,1) ${delay}ms` }
...
export default function Home() {
  ...
  // Refs for IntersectionObserver-based elements
  const [processRef, processInView] = useInView(0.3)
  ...
  const inputCls = 'w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-zinc-600 text-sm focus:outline-none focus:border-[#6366f1]/60 transition-colors'
  ...
  return (
    <div className="scanlines min-h-screen bg-[#050505] text-white overflow-x-hidden">
      ...
      {showTerminal && (
        <div
          className="fixed inset-0 z-[9990] bg-[#050505] flex items-center justify-center pointer-events-none transition-opacity duration-700"
      ...
      {/* ── Navbar ────────────────────────────────────────────────────────── */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${navScrolled ? 'border-b border-white/10 bg-[#050505]/90 backdrop-blur-md' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(l => (
              <a key={l.href} href={l.href} className="text-sm text-zinc-400 hover:text-white transition-colors duration-300">{l.label}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <a href="#contact" className="hidden md:inline-flex px-5 py-2 rounded-full bg-[#6366f1] text-white text-sm font-medium hover:bg-[#4f46e5] transition-all hover:shadow-lg hover:shadow-[#6366f1]/30">
              Получить предложение
            </a>
            <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg border border-white/10 text-zinc-400 hover:text-white" aria-label="Меню">
              {mobileOpen ? '✕' : '☰'}
            </button>
          </div>
        </div>
        {mobileOpen && (
          <div className="md:hidden border-t border-white/10 bg-[#050505] px-4 sm:px-6 py-5 flex flex-col gap-4">
            {NAV_LINKS.map(l => (
              <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className="text-zinc-300 hover:text-white py-1 transition-colors">{l.label}</a>
            ))}
            <a href="#contact" onClick={() => setMobileOpen(false)} className="mt-1 py-3 rounded-full bg-[#6366f1] text-white text-sm font-medium text-center">Получить предложение</a>
          </div>
        )}
      </nav>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 sm:px-6 pt-16 overflow-hidden">
        ...
        <div className="hidden lg:block absolute left-10 top-[35%] glass rounded-2xl px-4 py-3 text-sm font-medium shadow-2xl animate-float" style={{ animationDuration: '7s' }}>
          <span className="text-yellow-400 mr-2">⚡</span>Сайт готов за 4 дня
        </div>
        <div className="hidden lg:block absolute right-10 top-[30%] glass rounded-2xl px-4 py-3 text-sm font-medium shadow-2xl animate-float-alt" style={{ animationDuration: '9s' }}>
          <span className="text-green-400 mr-2">🚀</span>+40% конверсия
        </div>
        <div className="hidden lg:block absolute right-16 bottom-[30%] glass rounded-2xl px-4 py-3 text-sm font-medium shadow-2xl animate-float-slow" style={{ animationDuration: '11s' }}>
        ...
        <div className="relative flex flex-col items-center w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto mb-10 sm:mb-14 animate-fade-in-up" style={{ animationDelay: '1.5s' }}>
            <a href="#contact" className="btn-glow w-full sm:w-auto text-center px-8 py-4 rounded-full bg-[#6366f1] text-white font-semibold hover:bg-[#4f46e5] transition-all duration-300 text-base sm:text-lg hover:-translate-y-0.5">
              Обсудить проект →
            </a>
            <a href="#cases" className="w-full sm:w-auto text-center px-8 py-4 rounded-full border border-white/15 text-white font-semibold hover:border-white/30 hover:bg-white/5 transition-all duration-300 text-base sm:text-lg hover:-translate-y-0.5">
              Смотреть кейсы
            </a>
          </div>

          {/* Bento stats */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-lg sm:max-w-none mb-10 sm:mb-14 animate-fade-in-up" style={{ animationDelay: '1.7s' }}>
            {[{ v: '14 дней', l: 'среднее время запуска' }, { v: '3×', l: 'быстрее конкурентов' }, { v: '100%', l: 'клиентов рекомендуют' }].map(s => (
              <div key={s.v} className="glass rounded-2xl px-5 py-4 text-center">
      ...
      <section id="services" className="py-16 sm:py-28 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Услуги</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-4">Выберите свой формат</h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-lg mx-auto">Два пакета под разные задачи и бюджеты. Оба включают дизайн, разработку и запуск.</p></Reveal>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {SERVICES.map((s, i) => (
              <Reveal key={s.badge} delay={i * 100} className="h-full">
                <div className={`relative group rounded-2xl p-6 sm:p-8 border flex flex-col gap-5 h-full transition-all duration-500 hover:-translate-y-2 ${s.highlight ? 'border-[#6366f1]/60 bg-[#6366f1]/8 shadow-2xl shadow-[#6366f1]/10' : 'border-white/10 bg-white/[0.02] hover:border-[#6366f1]/40 hover:bg-[#6366f1]/5'}`}>
                  ...
                  <a href="#contact" className={`mt-2 py-3.5 rounded-full text-center text-sm font-semibold transition-all duration-300 ${s.highlight ? 'bg-[#6366f1] text-white hover:bg-[#4f46e5] shadow-lg shadow-[#6366f1]/30' : 'border border-white/15 text-white hover:border-[#6366f1]/50 hover:text-[#818cf8]'}`}>
                    Заказать {s.title}
                  </a>
                </div>
              </Reveal>
      ...
      <section id="process" className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5 bg-white/[0.01]">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Процесс</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-4">Как мы работаем</h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-12 max-w-md mx-auto">Прозрачный процесс от первого звонка до финального запуска</p></Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {PROCESS_STEPS.map((step, i) => (
              <Reveal key={step.num} delay={i * 110} className="h-full">
                <div className="flex flex-col items-center text-center gap-3 p-6 rounded-2xl border border-white/10 bg-white/[0.02] hover:border-[#6366f1]/30 hover:bg-[#6366f1]/5 transition-all duration-300 group h-full">
                  <div className="w-12 h-12 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center text-2xl group-hover:border-[#6366f1]/40 transition-colors">{step.icon}</div>
      ...
      <section className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Преимущества</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-4">Почему <span style={grad}>AI-разработка</span></h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-lg mx-auto">AI меняет скорость и стоимость разработки. Вы получаете качественный продукт за время, которое раньше занимало согласование ТЗ.</p></Reveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {WHY_AI.map((c, i) => (
              <Reveal key={c.title} delay={i * 80} className="h-full">
                <div className="group p-6 sm:p-7 rounded-2xl border border-white/10 bg-white/[0.02] hover:border-[#6366f1]/40 hover:bg-[#6366f1]/5 transition-all duration-300 flex gap-5 h-full">
                  <span className="text-3xl shrink-0 mt-0.5">{c.icon}</span>
                  <div>
                    <h3 className="font-bold text-lg mb-1.5 group-hover:text-[#818cf8] transition-colors duration-300">{c.title}</h3>
      ...
      <section id="cases" className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Кейсы</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-4">Реальные проекты</h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-md mx-auto">Результаты, которые говорят сами за себя</p></Reveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {CASES.map((c, i) => (
              <Reveal key={c.title} delay={i * 100} className="h-full">
                <div
                  className="group rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden hover:-translate-y-2 transition-all duration-500 flex flex-col h-full"
                  style={{
                    transform: `perspective(900px) rotateX(${tilts[i].rx}deg) rotateY(${tilts[i].ry}deg) translateY(0)`,
                    transition: tilts[i].rx === 0 && tilts[i].ry === 0 ? 'transform 0.5s ease' : 'transform 0.08s ease',
                  }}
                  ...
                  <div className="p-5 sm:p-6 flex flex-col flex-1">
                    ...
                    {c.image ? (
                      <a
                        ...
                        className="w-full py-2.5 rounded-full border border-white/10 text-zinc-400 text-sm font-medium text-center transition-all duration-300"
                      ...
                      <a
                        href="#contact"
                        className="w-full py-2.5 rounded-full border border-[#6366f1]/40 text-[#6366f1] text-sm font-medium text-center transition-all duration-300 hover:bg-[#6366f1] hover:text-white"
                      >
                        Обсудить проект →
                      </a>
                    )}
      ...
      <section id="why-site" className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5 bg-white/[0.01]">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Зачем сайт</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-3">Без сайта вы теряете клиентов<br /><span style={grad}>прямо сейчас</span></h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-lg mx-auto">73% людей ищут услуги в интернете перед покупкой. Вопрос лишь в том — найдут ли они вас.</p></Reveal>
          ...
          <Reveal delay={200}>
            <blockquote className="border-l-4 border-[#6366f1] pl-6 max-w-3xl mx-auto">
      ...
      <section id="conversion" className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Конверсия</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-3">Как работает конверсия</h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-lg mx-auto">От незнакомца до клиента — за одно посещение</p></Reveal>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 items-start">
            ...
            <Reveal delay={100}>
              <div className="flex flex-col gap-2">
                {FUNNEL_STEPS.map((step, i) => (
                  <div
                    key={i}
                    className="funnel-bar transition-all duration-300 hover:brightness-110 hover:-translate-y-0.5"
                    style={{ ['--fw' as string]: `${step.pct}%` }}
                  >
                    <div className="rounded-xl p-4 border"
                      style={{ background: `rgba(99,102,241,${0.06 + i * 0.03})`, borderColor: `rgba(99,102,241,${0.15 + i * 0.08})` }}>
                      <div className="flex items-center gap-3">
            ...
            <Reveal delay={200}>
              <div className="glass rounded-2xl p-6 sm:p-8">
                ...
                <a href="#contact" className="mt-5 block py-3 rounded-full bg-[#6366f1] text-white text-sm font-semibold text-center hover:bg-[#4f46e5] transition-all">
                  Хочу такой сайт →
                </a>
              </div>
            </Reveal>
      ...
      <section id="trust" className="py-16 sm:py-28 px-4 sm:px-6 border-t border-white/5 bg-white/[0.01]">
        <div className="max-w-6xl mx-auto">
          <Reveal><SectionLabel>Доверие</SectionLabel></Reveal>
          <Reveal delay={80}><h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-4">Нам доверяют потому что<br /><span style={grad}>мы прозрачны</span></h2></Reveal>
          <Reveal delay={160}><p className="text-zinc-400 text-center mb-10 sm:mb-16 max-w-md mx-auto">Никаких сюрпризов. Всё зафиксировано заранее.</p></Reveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Guarantees */}
            <Reveal delay={100} className="h-full">
                  ...
                  {FAQ_ITEMS.map((item, i) => (
                    <div key={i} className="border border-white/8 rounded-xl overflow-hidden">
                      <button
                        ...
                        className="w-full flex items-center justify-between gap-3 p-4 text-left text-sm hover:bg-white/5 transition-colors"
                      >
                        <span className="text-zinc-200 font-medium">{item.q}</span>
                        <span className={`text-[#6366f1] shrink-0 text-xl font-light transition-transform duration-300 ${faqOpen === i ? 'rotate-45' : ''}`}>+</span>
                      </button>
                      <div className={`overflow-hidden transition-[max-height] duration-300 ease-in-out ${faqOpen === i ? 'max-h-40' : 'max-h-0'}`}>
        ...
        <div className="max-w-5xl mx-auto relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 items-start">
            <Reveal>
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-5">Обсудим<br /><span style={grad}>ваш проект</span></h2>
      ...
      <footer className="py-12 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
          ...
          <div className="flex flex-col items-center md:items-end gap-2">
            <a href="mailto:lifescalekv@gmail.com" className="text-sm text-zinc-400 hover:text-[#6366f1] transition-colors">lifescalekv@gmail.com</a>
